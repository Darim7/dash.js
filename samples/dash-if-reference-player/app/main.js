    ...
    function ($scope, $window, sources, contributors, dashifTestVectors) {
        ...
        $scope.chartState = {
            audio: {
                ...
                mtp: {
                    ...
                    selected: false,
                    color: "#FFC400",
                    label: "Measured throughput (kpbs)",
                },
                etp: {
                    data: [],
                    selected: false,
                    color: "#1712B3",
                    label: "Estimated throughput (kpbs)",
                },
                liveLatency: {
                    data: [],
                    selected: false,
        ...
        $scope.player.on(
            ...
            function (e) {
                ...
                var availableRepresentations =
                    $scope.player.getRepresentationsByType(e.mediaType);
                var maxIndex = availableRepresentations
                    ? availableRepresentations.length
                    : 0;
                var pendingIndex = availableRepresentations.findIndex(function (
                    element
                ) {
        ...
        $scope.player.on(
            ...
            function (e) {
                var availableRepresentations =
                    $scope.player.getRepresentationsByType(e.mediaType);
                var index = availableRepresentations.findIndex(function (
                    element
                ) {
        ...
        $scope.player.on(
            ...
            function (e) {
                stopMetricsInterval();
                $scope.videoQualities =
                    $scope.player.getRepresentationsByType("video");
                $scope.chartCount = 0;
                ...
                streaming: {
                    buffer: {
                        mediaSourceDurationInfinity:
                            $scope.mediaSourceDurationInfinitySelected,
                    },
                },
            });
        };
                ...
                streaming: {
                    buffer: {
                        resetSourceBuffersForTrackSwitch:
                            $scope.resetSourceBuffersForTrackSwitch,
                    },
                },
            });
        };
            ...
            $scope.player.updateSettings({
                streaming: {
                    saveLastMediaSettingsForCurrentStreamingSession:
                        $scope.saveLastMediaSettingsSelected,
                },
            });
        };
        ...
        $scope.updateLowLatencyStallThreshold = function () {
            $scope.player.updateSettings({
                streaming: {
                    buffer: {
                        lowLatencyStallThreshold: parseFloat(
                            $scope.lowLatencyStallThreshold
                        ),
                    },
                },
            });
        ...
        $scope.updateInitialLanguageAudio = function () {
            var settings =
                $scope.player.getInitialMediaSettingsFor("audio") || {};
            $scope.player.setInitialMediaSettingsFor(
                "audio",
        ...
        $scope.updateInitialAccessibilityAudio = function () {
            ...
            $scope.player.setInitialMediaSettingsFor(
                "audio",
                Object.assign(settings, {
                    accessibility: $scope._genSettingsAudioAccessibility(
                        scheme,
                        $scope.initialSettings.audioAccessibility
                    ),
                })
            );
        };
        ...
        $scope.toggleImscEnableRollUp = function () {
            $scope.player.updateSettings({
                streaming: {
                    text: { imsc: { enableRollUp: $scope.imscEnableRollUp } },
                },
            });
        };
                    ...
                    text: {
                        imsc: {
                            displayForcedOnlyMode:
                                $scope.imscdisplayForcedOnlyMode,
                        },
                    },
                },
            });
        ...
        $scope.updateCmsdEtpWeightRatio = function () {
            $scope.player.updateSettings({
                streaming: {
                    cmsd: {
                        abr: {
                            etpWeightRatio: parseFloat(
                                $scope.cmsdEtpWeightRatio
                            ),
                        },
                    },
                },
        ...
        $scope.doLoad = function () {
            ...
            if ($scope.selectedItem.hasOwnProperty("bufferConfig")) {
                ...
                if (selectedConfig.bufferTimeDefault) {
                    config.streaming.buffer.bufferTimeDefault =
                        selectedConfig.bufferTimeDefault;
                }
            ...
            const lowLatencyStallThreshold = parseFloat(
                $scope.lowLatencyStallThreshold
            );
            if (!isNaN(lowLatencyStallThreshold)) {
                config.streaming.buffer.lowLatencyStallThreshold =
            ...
            if (scheme !== "off") {
                audioSettings.accessibility =
                    $scope._genSettingsAudioAccessibility(
                        scheme,
                        $scope.initialSettings.audioAccessibility
                    );
        ...
        $scope.copyQueryUrl = function () {
            var currentExternalSettings = {
                ...
                accessibilitySchemeAudio:
                    $scope.initialSettings.audioAccessibilityScheme,
        ...
        function calculateHTTPMetrics(type, requests) {
            ...
            if (requestWindow.length > 0) {
                ...
                etp[type] =
                    request.cmsd &&
                    request.cmsd.dynamic &&
                    request.cmsd.dynamic.etp
                        ? request.cmsd.dynamic.etp
                        : 0;
        ...
        function updateMetrics(type) {
            ...
            if (dashMetrics && $scope.currentStreamInfo) {
                ...
                var representations =
                    $scope.player.getRepresentationsByType(type);
                ...
                if ($scope.player.getCurrentRepresentationForType(type)) {
                    var index =
                        $scope.player.getCurrentRepresentationForType(type)
                            .absoluteIndex + 1;
                }
                ...
                if ($scope.chartCount % 2 === 0) {
                    ...
                    if (httpMetrics) {
                        ...
                        $scope.plotPoint(
                            "etp",
                            type,
                            (httpMetrics.etp[type] / 1000).toFixed(3),
                            time
                        );
                        $scope.plotPoint(
                            "mtp",
        ...
        function setLatencyAttributes() {
            ...
            $scope.defaultStableBufferDelay =
                currentConfig.streaming.buffer.bufferTimeDefault;
            $scope.defaultBufferTimeAtTopQuality =
        ...
        function setAbrRules() {
            var currentConfig = $scope.player.getSettings();
            $scope.activeAbrRules.throughputRule =
                currentConfig.streaming.abr.rules.throughputRule.active;
            $scope.activeAbrRules.bolaRule =
                currentConfig.streaming.abr.rules.bolaRule.active;
            $scope.activeAbrRules.insufficientBufferRule =
                currentConfig.streaming.abr.rules.insufficientBufferRule.active;
            $scope.activeAbrRules.switchHistoryRule =
                currentConfig.streaming.abr.rules.switchHistoryRule.active;
            $scope.activeAbrRules.droppedFramesRule =
                currentConfig.streaming.abr.rules.droppedFramesRule.active;
            $scope.activeAbrRules.abandonRequestsRule =
                currentConfig.streaming.abr.rules.abandonRequestsRule.active;
            $scope.activeAbrRules.loLPRule =
                currentConfig.streaming.abr.rules.loLPRule.active;
            $scope.activeAbrRules.l2ARule =
                currentConfig.streaming.abr.rules.l2ARule.active;
        }
        ...
        function setAdditionalPlaybackOptions() {
            ...
            $scope.applyContentSteering =
                currentConfig.streaming.applyContentSteering;
            ...
            $scope.mediaSourceDurationInfinitySelected =
                currentConfig.streaming.buffer.mediaSourceDurationInfinity;
            $scope.resetSourceBuffersForTrackSwitch =
                currentConfig.streaming.buffer.resetSourceBuffersForTrackSwitch;
            $scope.saveLastMediaSettingsSelected =
                currentConfig.streaming.saveLastMediaSettingsForCurrentStreamingSession;
        ...
        function setTextOptions() {
            var currentConfig = $scope.player.getSettings();
            $scope.imscEnableRollUp =
                currentConfig.streaming.text.imsc.enableRollUp;
            $scope.imscdisplayForcedOnlyMode =
                currentConfig.streaming.text.imsc.displayForcedOnlyMode;
        }
        ...
        function setStallThresholdOptions() {
            var currentConfig = $scope.player.getSettings();
            $scope.stallThreshold =
                currentConfig.streaming.buffer.stallThreshold;
            $scope.lowLatencyStallThreshold =
                currentConfig.streaming.buffer.lowLatencyStallThreshold;
        }
        ...
        function setInitialSettings() {
            ...
            var currentAudioSettings =
                $scope.player.getInitialMediaSettingsFor("audio");
            if (currentAudioSettings) {
                $scope.initialSettings.audioLang = currentAudioSettings.lang;
                if (currentAudioSettings.role) {
                    $scope.initialSettings.audioRole =
                        currentAudioSettings.role.value;
                }
                if (currentAudioSettings.accessibility) {
                    $scope.initialSettings.audioAccessibilityScheme =
                        $scope._backconvertRoleScheme(
                            currentAudioSettings.accessibility
                        );
                    $scope.initialSettings.audioAccessibility =
                        currentAudioSettings.accessibility.value;
                } else {
                    $scope.initialSettings.audioAccessibilityScheme =
                        $scope._backconvertRoleScheme(null);
                }
            }
        ...
        function setCMSDSettings() {
            ...
            $scope.cmsdEtpWeightRatio =
                currentConfig.streaming.cmsd.abr.etpWeightRatio;
        }
